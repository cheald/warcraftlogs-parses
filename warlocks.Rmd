...
```
$ API_KEY=your_key_here ruby grabber.rb warlock
```

Once we have the JSON files, we use a small script to boil them down into a CSV:

```
$ API_KEY=your_key_here ruby distill.rb warlock
```

This results in the `csv/warlock.csv` that we then consume in R.
