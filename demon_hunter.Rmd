---
title: "Demon Hunter spec scaling"
output:
  html_notebook: 
    fig_height: 6
    fig_width: 18
...
```
$ API_KEY=your_key_here ruby grabber.rb demon_hunter
```

Once we have the JSON files, we use a small script to boil them down into a CSV:

```
$ API_KEY=your_key_here ruby distill.rb demon_hunter
```

This results in the `csv/demon_hunter.csv` that we then consume in R.
...
library(dplyr)
library(broom)
data <- read.csv("csv/demon_hunter.csv")
